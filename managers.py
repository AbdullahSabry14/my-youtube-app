...
if not URL :
    ...
    st.markdown("<p style='text-align: center;'>قم بربط قناتك للحصول على رابط الرفع الخ اص بك</p>", unsafe_allow_html=True)
    ...
    # code = st.query_params.get("code")
    # if code:
    #     flow = Flow.from_client_secrets_file(json.loads(st.secrets["G_CRED"]), scopes, redirect_uri="http://192.168.88.8:8501/")
    #     flow.fetch_token(code=code)
    #     creds = flow.credentials
else :
    # --- الشاشة الجانبية ---
